...
// EDIT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  ...
  int pbBounce = 300; //300 millisecond debounce duration - you may want to change this value depending on the mechanics of your pushbuttons
  ...
  int toReadAnalogTeensy[7] = { 
           //Pin number are written below
  0,0,0,0, //38,39,40,41 (A0,A1,A2,A3)
  0,0,0    //42,43,44 (A4,A5,A6)
  };
  //THRESHOLD
...
void setup(){ 
  ...
  for(int i = 0; i < 92; i ++){
    buttonState[i]=1; //all pushbuttons are off
  }
...
void loop(){
  ...
  for(int i=0;i<6;i++){ //loop through all encoders
    if(toReadEncoder[i]==true){ //check if we should read this encoder
      ...
      if(shiftChange==46){ //shift button is engaged, encoders 6-11  
        int k=i+6; 
        if(tempEncPosition > encPosition[k]){ //this position is greater than the last
            midiSend('e',1,k); //send message
            encPosition[k] = tempEncPosition; //update position          
        }else if(tempEncPosition < encPosition[k]){ //this position is less than the last
            midiSend('e',0,k); //send message
            encPosition[k] = tempEncPosition; //update position          
        }else{
          //do nothing  
        }    
      }else{  //shift button is not engaged, encoders 0-5  
...
//  }
  if(buttonState[28]==0){ //button 28 is pressed
    analogWrite(14,66); //LED 14 is on at 25% brightness 
  }else{ 
    analogWrite(14,0); //LED 14 is off
  }
  if(buttonState[29]==1){ //button 29 is off
    analogWrite(15,255);  //LED 15 is at 100% brightness
  }else{ 
    analogWrite(15,0); //LED 15 is off
  }
...
void midiSend(char type, int val, int pin){ 
  ...
  case 'p': //--------------- PUSHBUTTON   
    if(enableDebug){
      ...
      if(val==1){
        //___PUSHBUTTON MIDI MESSAGE (ON)
        //usbMIDI.sendNoteOn(pin,127,channelNumber); //!!!
      }else{
        //___PUSHBUTTON MIDI MESSAGE (OFF)
  ...
  case 'e': //--------------- ENCODER
    if(enableDebug){
      ...
      if(val==1){
        //___ENCODER MIDI MESSAGE (DIRECTION 1)
        //note
        ...
        //___ENCODER MIDI MESSAGE (DIRECTION 2)
        //note
